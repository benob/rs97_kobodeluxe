...
void prefs_t::init()
{
	...
	key("depth", depth, 16); desc("Display Depth");
	key("maxfps", max_fps, 60); desc("Maximum fps");
	key("maxfpsstrict", max_fps_strict, 0); desc("Strictly Regulated fps");
	key("buffer", doublebuf, 0); desc("Display Buffer Mode");
	yesno("shadow", shadow, 1); desc("Use Software Shadow Buffer");
	key("videomode", videomode, 0x04301); desc("Video Mode");
	yesno("vsync", vsync, 1); desc("Enable Vertical Sync");
	key("videopages", pages, -1); desc("Number of Video Pages");

	comment("--- Graphics settings ----------------------");
	key("scalemode", scalemode, GFX_SCALE_NEAREST); desc("Scaling Filter Mode");
	yesno("dither", use_dither, 0); desc("Use Dithering");
