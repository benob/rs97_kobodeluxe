...
int gfxengine_t::show()
{
    ...
    _doublebuf = 0;
    _fullscreen = 1;
    flags = SDL_DOUBLEBUF | (_fullscreen ? SDL_FULLSCREEN : 0) | SDL_SWSURFACE;
	screen_surface = SDL_SetVideoMode(_width, _height * 2, _depth, flags);
	if(!screen_surface)
	{
...
void SDL_BlitSurface_2x(SDL_Surface* source, SDL_Rect* source_rect, SDL_Surface* dest, SDL_Rect* dest_rect) {
    //printf("%d %d %d %d | %d %d %d %d\n", source_rect->x, source_rect->y, source_rect->w, source_rect->h, dest_rect->x, dest_rect->y, dest_rect->w, dest_rect->h);
    //printf("%d %d | %d %d\n", source->w, source->h, dest->w, dest->h);
    SDL_LockSurface(source);
    SDL_LockSurface(dest);
    /*for(int j = 0; j < source_rect->h; j++) {
        for(int i = 0; i < source_rect->w; i += 2) {
            ((unsigned short*) dest->pixels)[(dest_rect->x + i) >> 1 + (dest_rect->y + j) * dest->pitch / 2] = 
                ((unsigned short*) source->pixels)[source_rect->x + i + (source_rect->y + j) * source->pitch / 2];
        }
    }*/
    for(int j = 0; j < source->h; j++) {
        memcpy((char*)dest->pixels + (j * 2) * dest->pitch, (char*)source->pixels + j * source->pitch, dest->w * 2);
        memcpy((char*)dest->pixels + (j * 2 + 1) * dest->pitch, (char*)source->pixels + j * source->pitch, dest->w * 2);
    }
...
void gfxengine_t::flip()
{
	...
	{
		/*for(i = 0; i < dirtyrects[backpage]; ++i)
			SDL_BlitSurface_2x(softbuf,
					&dirtytable[backpage][i],
					screen_surface,
					&dirtytable[backpage][i]);*/
        SDL_BlitSurface_2x(softbuf, NULL, screen_surface, NULL);
	}
